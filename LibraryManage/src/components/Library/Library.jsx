...
function Library() {
 const { name,setname,totalfootfall,todayfootfall } = useOutletContext();
  ...
  // Add state for welcome/thank you messages
  ...
  const showAnimatedMessage = (text, duration = 3000) => {
    setMessage(text);
    setShowMessage(true);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (data.success===true) {
        ...
        showAnimatedMessage("Welcome! 🎉", 2000);
      } else {
        toast.error(data.message || "Something went wrong");
        showAnimatedMessage("Please try again", 2000);
      }
  ...
  const handleSubmit2=async (e)=>{
    ...
    if(data.message==="success"){
      ...
      showAnimatedMessage("Thank you! 👋", 2000);
    }else{
      toast.error("student can't be deleted");
      showAnimatedMessage("Operation failed", 2000);
    }
      
    } catch (error) {
      toast.error("Something went wrong")
      showAnimatedMessage("Error occurred", 2000);
    }finally{
      setloading(false)
    }
  ...
  return (
    <> <div className="min-h-screen bg-gradient-to-br from-slate-900 to-gray-500">
      ...
      {/* Animated Welcome/Thank You Message */}
      {showMessage && (
        <div className="flex justify-center items-center mb-6">
          <div className={`
            ...
            ${showMessage ? 'scale-100 opacity-100 translate-y-0' : 'scale-95 opacity-0 translate-y-4'}
            animate-pulse
          `}>
            <div className="flex items-center space-x-3">
              <div className="text-2xl animate-bounce">
                {message.includes('Welcome') ? '🎉' : message.includes('Thank') ? '👋' : '💫'}
              </div>
